...
pub struct TestsStatus {
    ...
    /// Overall verdict of the project's checks when one was reported.
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
pub struct VerificationContext {
    ...
    /// Explicit verdict of the project's own checks (check_work): `Some(true)`
    /// passed, `Some(false)` failed, `None` not run / no project detected.
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
impl Verifier {
    ...
    pub fn verify(&self, ctx: &VerificationContext) -> VerificationReport {
        ...
        let eval_ctx = EvaluationContext {
            ...
            // call that may have run them is the fallback signal.
            tests_run: ctx.tests_passed.is_some()
                || tools_called
                    .iter()
        ...
        let mut tests_status = self.check_tests(
            &ctx.changed_files,
            &ctx.tests_added,
            eval_ctx.tests_run,
            &ctx.tool_calls,
        );
        tests_status.tests_ok = ctx.tests_passed;
        ...
        if ctx.tests_passed == Some(false) {
            missing_evidence.push(
                "The project's checks (check_work) failed — fix them before finishing.".into(),
            );
        }
    ...
    pub fn format_report(report: &VerificationReport) -> String {
        ...
        out.push_str(&format!(
            "- **Tests run**: {}\n\n",
            match (report.tests_status.tests_run, report.tests_status.tests_ok) {
                (true, Some(true)) => "yes (passed)",
                (true, Some(false)) => "yes (FAILED)",
                (true, None) => "yes",
                (false, _) => "no",
            }
        ));
...
mod tests {
    ...
    #[test]
    fn explicit_tests_passed_drives_tests_dimension() {
        let verifier = Verifier::new("/tmp");
        // No shell calls at all: only the explicit verdict says tests ran.
        let passed = verifier.verify(&VerificationContext {
            task_description: "verify".into(),
            tests_passed: Some(true),
            ..Default::default()
        });
        assert!(passed.tests_status.tests_run);
        assert_eq!(passed.tests_status.tests_ok, Some(true));
        assert_eq!(passed.status, VerificationStatus::Passed);
        assert!(Verifier::format_report(&passed).contains("Tests run**: yes (passed)"));

        let failed = verifier.verify(&VerificationContext {
            task_description: "verify".into(),
            tests_passed: Some(false),
            ..Default::default()
        });
        assert!(failed.tests_status.tests_run);
        assert_eq!(failed.status, VerificationStatus::PassedWithWarnings);
        assert!(failed
            .missing_evidence
            .iter()
            .any(|m| m.contains("check_work")));
        assert!(Verifier::format_report(&failed).contains("yes (FAILED)"));

        let unknown = verifier.verify(&VerificationContext {
            task_description: "verify".into(),
            ..Default::default()
        });
        assert!(!unknown.tests_status.tests_run);
        assert_eq!(unknown.tests_status.tests_ok, None);
        assert!(Verifier::format_report(&unknown).contains("Tests run**: no"));
    }
